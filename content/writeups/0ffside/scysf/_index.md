---
title: So Close Yet So Far
date: 2026-06-14T00:00:00.000+01:00
display:
  showAuthor: true
  showReadingTime: true
  showWordCount: true
  showPagination: true
featureimage: https://i.pinimg.com/236x/d2/f1/8d/d2f18d43ad7a77c14df6438a0f819430.jpg?nii=t
draft: false
---
{{< alert icon="flag" cardColor="rgba(var(--color-primary-500),0.1)" iconColor="rgb(var(--color-primary-400))" textColor="inherit" >}}
**So Close Yet So Far** &nbsp;·&nbsp; `Misc`

A very very tragic scenario.
**Flag Format : 0ffside{HomeTeam_AwayTeam_Player_Agg-Score_Season}**
{{< /alert >}}

Writeup for "[So Close Yet So Far](https://0ffsidectf.ddns.net/challenges)"
